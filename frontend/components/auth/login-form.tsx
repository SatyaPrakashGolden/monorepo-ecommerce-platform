...
export function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3️⃣ Prepare login payload with FCM token
      ...
      if (fcmToken) {
        loginPayload.token = fcmToken
        loginPayload.deviceType = deviceType
      }

      // 4️⃣ Single login request with FCM token
      const response = await fetch("http://localhost:2000/api/user/login", {
        method: "POST",
